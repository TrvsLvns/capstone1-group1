...
public class Player : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        hit = Physics2D.BoxCast(transform.position,boxCollider.size, 0, new Vector2(0,moveDelta.y), Mathf.Abs(moveDelta.y * 2 * Time.fixedDeltaTime), LayerMask.GetMask("Actor","Blocking"));
        if (hit.collider == null)
        {
            // Actually do the y movement
            transform.Translate(0, moveDelta.y * 2 * Time.fixedDeltaTime, 0); // Time.deltaTime smooths out performance between devices
        }

        // Check if we can move in the x direction
        hit = Physics2D.BoxCast(transform.position, boxCollider.size, 0, new Vector2(moveDelta.x, 0), Mathf.Abs(moveDelta.x * 2 * Time.fixedDeltaTime), LayerMask.GetMask("Actor", "Blocking"));
        if (hit.collider == null)
        {
            // Actually do the Xy movement
            transform.Translate(moveDelta.x * 2 * Time.fixedDeltaTime, 0, 0); // Time.deltaTime smooths out performance between devices
        }
    }
